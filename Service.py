...
def reload_game(engine, hero):
    ...
    if not generator['obj'].objects:
        engine.add_objects(generator['obj'].get_objects(_map))
    else:
        engine.add_objects(generator['obj'].objects)
...
def add_experience(engine, hero):
    engine.score += 1
    hero.exp += hero.level * 500
    engine.notify("Experience up!")
    hero.level_up()
...
def add_gold(engine, hero):
    ...
    if chance == 1:
        ...
        engine.notify("You drank the poison")  
    else:
        engine.score += 0.1
