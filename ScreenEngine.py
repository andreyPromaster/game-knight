...
class GameSurface(ScreenHandle):
    ...
    def calculate (self, min_x, min_y):
        size_x_map = len(self.game_engine.map[0])
        size_y_map = len(self.game_engine.map)

        if size_y_map - self.game_engine.hero.position[1] <= 3:
            min_y = size_y_map - 7 - 1
        else:
            min_y = self.game_engine.hero.position[1] - 4

        if size_x_map - self.game_engine.hero.position[0] <= 5:
            min_x = size_x_map - 10 - 1
        else:
            min_x = self.game_engine.hero.position[0] - 5
        
        if self.game_engine.hero.position[0] <= 5:
            min_x = 0
        if self.game_engine.hero.position[1] <= 3:
            min_y = 0
